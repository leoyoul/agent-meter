...
const updateState = ref<AppUpdateState>({ phase: 'idle', currentVersion: appVersion, downloadedBytes: 0 })
...
async function checkAndInstallUpdate() { if (['checking', 'downloading'].includes(updateState.value.phase)) return; await checkForUpdates(false); if (updateState.value.phase === 'available') await downloadAndRestart() }
...
onMounted(async () => {
  ...
  if (!isSettingsView) unlisteners.push(await meterApi.on('update-requested', () => { void checkAndInstallUpdate() }))
  ...
  if (!isSettingsView && meterApi.isTauri()) { void checkForUpdates(true); updateInterval = setInterval(() => { void checkForUpdates(true) }, 86_400_000) }
})
onBeforeUnmount(() => { disposed = true; dashboardReloadRequested = false; unlisteners.splice(0).forEach(fn => fn()); if (updateInterval) clearInterval(updateInterval); if (metricsRefreshDelay) clearTimeout(metricsRefreshDelay) })
</script>
...
<template>
  <main v-if="!isSettingsView" class="app-shell">
    ...
    <section v-if="!sourceUnavailable" class="kpi-grid" aria-label="四大指标">
      <article class="kpi speed"><div class="metric-code">速</div><span>{{ periodLabel }}平均有效 TPS</span><strong>{{ formatTps(summary?.averageEffectiveTps) }}</strong><small>{{ summary?.tpsSampleCount ?? 0 }} 个 TPS 样本</small></article>
      <article class="kpi first"><div class="metric-code">首</div><span>{{ periodLabel }}平均首响</span><strong>{{ formatDuration(summary?.averageTtftMs) }}</strong><small>{{ summary?.ttftSampleCount ?? 0 }} 个首响样本</small></article>
    ...
    <section v-if="!sourceUnavailable" class="panel matrix-panel"><div class="panel-heading"><div><span class="eyebrow">核心分析</span><h2>来源 × 模型 × 推理强度</h2></div><span class="coverage"><i :class="{ partial: !summary?.pricing.complete }"></i>费用覆盖 {{ pricedPercent }}%</span></div><div class="table-scroll"><table><thead><tr><th>来源</th><th>模型 / Provider</th><th>推理强度</th><th>调用</th><th>速</th><th>首</th><th>量</th><th>费</th></tr></thead><tbody><tr v-for="row in stats" :key="`${row.sourceId}-${row.model}-${row.reasoningEffort}`"><td><span class="source-badge">{{ row.sourceName }}</span></td><td><strong>{{ row.model }}</strong><small>{{ row.provider }}</small></td><td><span class="effort-badge">{{ effortName(row.reasoningEffort) }}</span></td><td>{{ row.callCount }}<small>首响 {{ row.ttftSampleCount }} · TPS {{ row.tpsSampleCount }}</small></td><td>{{ formatTps(row.averageEffectiveTps) }}</td><td>{{ formatDuration(row.averageTtftMs) }}</td><td>{{ formatCompact(row.tokens.total) }}</td><td><strong>{{ formatCost(row.estimatedCostNanoUsd, row.pricing.complete) }}</strong><small>{{ Math.round(row.pricing.ratio * 100) }}% 覆盖</small></td></tr><tr v-if="!stats.length"><td colspan="8"><div class="empty">当前筛选没有可用数据</div></td></tr></tbody></table></div></section>
  ...
  <main v-else class="settings-page" aria-label="Agent Meter 设置"><header class="settings-header"><div class="brand"><div class="brand-mark"><img :src="brandLogo" alt="" /></div><div><strong>Agent Meter 设置</strong><span>菜单栏、数据源与应用更新</span></div></div><button class="version-button" :disabled="['checking','downloading'].includes(updateState.phase)" title="检查并安装更新" @click="checkAndInstallUpdate">v{{ updateState.currentVersion }}</button></header>
    <div v-if="error" class="notice error"><CircleAlert :size="18" />{{ error }}<button aria-label="关闭错误" @click="error = ''"><X :size="16" /></button></div>
    <div v-if="lastAction" class="notice success"><Check :size="18" />{{ lastAction }}<button aria-label="关闭提示" @click="lastAction = ''"><X :size="16" /></button></div>
      ...
      <section><h3>软件更新</h3><div class="source-row"><div class="source-icon"><CloudDownload :size="19"/></div><div><strong>自动检查稳定版</strong><small>每次启动检查，运行中每 24 小时检查一次</small></div><span class="update-policy">始终开启</span></div><div class="settings-card"><div class="card-heading"><div><strong>Agent Meter {{ updateState.currentVersion }}</strong><small>当前 v{{ updateState.currentVersion }} · 目标 {{ updateState.platform || '未识别' }} · {{ appSettings.updates.lastCheckedAt ? `上次检查 ${formatTime(appSettings.updates.lastCheckedAt)}` : '尚未检查更新' }}</small></div><button class="secondary-button" :disabled="['checking','downloading'].includes(updateState.phase)" @click="checkAndInstallUpdate"><LoaderCircle v-if="updateState.phase === 'checking'" :size="15" class="spin"/><RefreshCw v-else :size="15"/>检查并安装</button></div><div v-if="updateState.phase === 'available'" class="update-result"><strong>发现 {{ updateState.version }}</strong><p>{{ updateState.notes }}</p><button class="primary-button" @click="downloadAndRestart"><CloudDownload :size="16"/>下载并重启</button></div><div v-else-if="updateState.phase === 'downloading' || updateState.phase === 'ready'" class="update-result"><span>{{ updateState.phase === 'ready' ? '安装完成，正在重启' : `正在下载并验证 ${Math.round(updateProgress)}%` }}</span><progress :value="updateState.downloadedBytes" :max="updateState.totalBytes || 1"></progress></div><div v-else-if="updateState.phase === 'current'" class="inline-result success-text"><Check :size="15"/>{{ updateState.message || '已确认是最新版' }}</div><div v-else-if="updateState.phase === 'unavailable'" class="inline-result unavailable-text"><Info :size="15"/>{{ updateState.message || '当前环境不支持应用更新检查' }}</div><div v-else-if="updateState.phase === 'error'" class="inline-result error-text"><CircleAlert :size="15"/>检查更新失败：{{ updateState.error }}</div></div></section>
