...
export interface MetricSummary { period: MetricPeriod; callCount: number; ttftSampleCount: number; tpsSampleCount: number; averageTtftMs: number | null; averageEffectiveTps: number | null; tokens: UsageTokens; estimatedCostNanoUsd: number; pricing: PricingCoverage; lastUpdatedAt: string | null }
export interface MetricSeriesPoint { bucket: string; label: string; callCount: number; ttftSampleCount: number; tpsSampleCount: number; averageTtftMs: number | null; averageEffectiveTps: number | null; totalTokens: number; estimatedCostNanoUsd: number }
export interface ModelEffortStat { sourceId: number; sourceName: string; provider: string; model: string; reasoningEffort: string; callCount: number; ttftSampleCount: number; tpsSampleCount: number; averageTtftMs: number | null; averageEffectiveTps: number | null; tokens: UsageTokens; estimatedCostNanoUsd: number; pricing: PricingCoverage }
